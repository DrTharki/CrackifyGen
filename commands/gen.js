...
module.exports.run = (bot, message, args, gen) => {
    ...
    if(!gen.allowedChannel(message.channel)) return message.channel.send({embed:{title:`Gen can't be used in this channel!`,description:`Please go to gen channel` , color:bot.color}})
    ...
    if(!alt || alt === '') return message.channel.send({embed:{title:`We don't have this Account Type in stock!`, color:bot.color}})
    let embed = new Discord.RichEmbed()
    .setColor(bot.color)
    .setTitle('Nitro GEN')
    .addField('Account Type', type)
    .addField('Account',alt.split(':')[0],true)
    .addField('Password',alt.split(':')[1],true)
    .addField('Website', alt.split(':')[2], true)
