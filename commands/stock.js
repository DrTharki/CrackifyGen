...
/*let embedTemp = new Discord.RichEmbed()
.setTitle('Stock')
.setColor(bot.color);*/
gen.calculateStock()
setTimeout(() => {
...
let embedFields=0;
for(let i=0;i<stockArr.length;i++){embedFields++;if(embedFields>24){if(!embed[embed.length])embed[embed.length]=new Discord.RichEmbed().setTitle('Stock').setColor(bot.color);embedFields=0;}
